...
client.user.setGame(`Great Bot *help `,"https://www.twitch.tv/rashed1059")
  console.log('')
  console.log('')
  console.log('?[?????????????????????????????????????????????????????????????????]?')
 ...
 if (message.content === "*invite") {
  const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setFooter('Great B0T: جميع الحقوق محفوظة 2018 لــبوت')  
      .addField('Bot Link', `  https://discordapp.com/api/oauth2/authorize?client_id=601538272161693697&permissions=8&scope=bot  `)
  message.author.send({embed});
